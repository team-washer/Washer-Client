...
import { History, Clock, Calendar, CheckCircle, XCircle, Play, Loader2 } from "lucide-react"
import { machineApi, type MachineHistoryItem } from "@/lib/api-client"
...
export function MachineHistoryModal({ machineId, machineName }: MachineHistoryModalProps) {
  ...
  const loadHistory = async () => {
    ...
    try {
      ...
      toast({
        title: "히스토리 로드 실패",
        description: error.message || "기기 히스토리를 불러오는 중 오류가 발생했습니다.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  const getTimelineEvents = (item: MachineHistoryItem) => {
    ...
    if (item.confirmedAt) {
      events.push({ time: item.confirmedAt, label: "예약 확정", icon: CheckCircle })
    }
    if (item.startedAt) {
      events.push({ time: item.startedAt, label: "작업 시작", icon: Play })
    }
    if (item.completedAt) {
      events.push({ time: item.completedAt, label: "작업 완료", icon: CheckCircle })
    }
    if (item.cancelledAt) {
      events.push({ time: item.cancelledAt, label: "예약 취소", icon: XCircle })
    }
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>최근 사용 기록을 확인할 수 있습니다.</DialogDescription>
        </DialogHeader>
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <span className="font-medium">{item.machineLabel}</span>
                        </div>
                      ...
                      <div className="space-y-3">
                        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">진행 과정</h4>
